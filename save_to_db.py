...
def get_db_connection():
    ...
    if database_url:
        ...
        try:
            ...
            # But disable SSL when connecting through localhost tunnel
    ...
    try:
        ...
        print("  Option 1: DATABASE_URL=postgresql://user:password@host:port/database")
        print("  Option 2: Individual environment variables:")
...
def normalize_patient_record(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized = {
        'emr_id': emr_id.strip() if (isinstance(emr_id, str) and emr_id) else (emr_id if emr_id else None),
        ...
        'status': record.get('status') or record.get('patient_status') or record.get('status_class') or None
    }

    for key, value in list(normalized.items()):
        # Skip datetime objects - they're already handled
...
def normalize_timestamp(timestamp_str: str) -> Optional[datetime]:
    ...
    if not timestamp_str or (isinstance(timestamp_str, str) and timestamp_str.strip() == ''):
        return None
    
    timestamp_str = timestamp_str.strip() if isinstance(timestamp_str, str) else str(timestamp_str)
