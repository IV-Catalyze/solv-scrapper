        ...
        cursor.execute(
            ...
            SELECT 
                ...
                COALESCE(
                    ...
                    q.raw_payload->'createdByUser'->>'emailAddress',
                    ...
                    e.encounter_payload->'createdByUser'->>'emailAddress',
    ...
    <script>
        ...
        try:
            ...
            for v_result in validation_results:
                ...
                if complaint_id_from_db:
                    ...
                    # Extract manual validation field_validations and screenshot_field_validations if exists
        ...
        return templates.TemplateResponse(
...

            {
                ...
                "has_summary_image": has_summary_image,
...
async def save_manual_validation(
    ...
    try:
        ...
        try:
            ...
            # Calculate screenshot validation overall status (if provided)
            screenshot_overall_status = None
            screenshot_correct_count = 0
            screenshot_total_count = 0
            if screenshot_field_validations:
                screenshot_all_values = list(screenshot_field_validations.values())
                screenshot_correct_count = sum(1 for v in screenshot_all_values if v == 'correct')
                screenshot_total_count = len(screenshot_all_values)
                
                if screenshot_correct_count == screenshot_total_count:
                    screenshot_overall_status = "PASS"
                elif screenshot_correct_count == 0:
                    screenshot_overall_status = "FAIL"
                else:
                    screenshot_overall_status = "PARTIAL"
            ...
            # Add screenshot validation data if provided
            if screenshot_field_validations:
                manual_validation_result["manual_validation"]["screenshot_field_validations"] = screenshot_field_validations
                manual_validation_result["screenshot_field_summary"] = {
                    "total_fields": screenshot_total_count,
                    "correct_fields": screenshot_correct_count,
                    "incorrect_fields": screenshot_total_count - screenshot_correct_count
                }
...
@router.get(
    "/queue/validation/{encounter_id}/image/summary",
    tags=["Queue"],
    summary="Get Summary screenshot image",
    include_in_schema=False,
    responses={
        200: {"description": "Image retrieved successfully"},
        404: {"description": "Image not found"},
...
async def get_summary_image(
    ...
    """
    Get the Summary screenshot image for an encounter.
    Uses session authentication for UI access.
    """
    try:
        # Import helper functions from routes.py module to avoid circular imports
        ...
        # Find Summary image path using encounter_id
        summary_image_path = find_encounter_image(encounter_id, "summary")

        if not summary_image_path:
            raise HTTPException(
                status_code=404,
                detail=f"Summary image not found for encounter_id: {encounter_id}"
            )

        # Get image bytes
        image_bytes = get_image_bytes_from_blob(summary_image_path)

        if not image_bytes:
            raise HTTPException(
                status_code=404,
                detail=f"Failed to load image from: {summary_image_path}"
            )

        # Determine content type from file extension
        content_type = get_content_type_from_blob_name(summary_image_path)
        ...
        logger.error(f"Error fetching Summary image for encounter_id {encounter_id}: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail=f"Internal server error: {str(e)}"
        )
